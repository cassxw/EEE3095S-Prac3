...
int main(void)
{
  ...
  while (1)
  {
	...
	char adc_val_str[20];
	sprintf(adc_val_str, "ADC Value: %lu", adc_val);
...
void EXTI0_1_IRQHandler(void)
{
	...
	// This function is called every time PA0 is pressed.

	// Get the current time in ms
	curr_millis = HAL_GetTick();

	// Debouncing on PA0: time waiting set to 100ms
	// If 100ms has passed since last PA0 press...
	if ((curr_millis - prev_millis) >= 100) {

		// Toggle the flashing LED (PB7) between 1 Hz and 2 Hz.
		// 1 Hz = 1000 ms
		// 2 Hz = 500 ms

		// If currently set to 2 Hz, change to 1 Hz.
		if (delay_t == 500) {
			delay_t = 1000;
...
uint32_t ADCtoCCR(uint32_t adc_val){
	...
	// ADC is configured in 12-bit mode.
	// ARR = 47999
	// ...for a 1 kHz PWM signal.
